...
# ---------- Slide 5
    ...
    st.subheader("📌 Tasa de Respuesta")
    st.write(tasa_respuesta)
...
# ---------- Slide 6
    ...
    top_estados = df.groupby("State")["Customer Lifetime Value"].sum().nlargest(5)
    st.subheader("🌎 Top 5 Estados por CLV Total")
    st.write(top_estados)
    participacion = (top_estados.sum() / df["Customer Lifetime Value"].sum()) * 100
...
# ---------- Slide 7
    ...
    st.subheader("📈 Correlación CLV vs Ingreso")
    if corr > 0.6:
