...
with st.expander("ℹ️ Instrucciones rápidas", expanded=False):
    st.markdown("""
- **Paso 1:** Sube `CLV.csv` (o colócalo en la raíz del repositorio).
- **Paso 2:** En la barra lateral, mapea las columnas (**ID**, **CLV**, **Fecha** y opcionales).
- **Paso 3:** Navega por los *Slides* para explorar la distribución, Pareto, evolución temporal y la relación **CLV vs Frecuencia**.
""")
...
# % de clientes necesario para alcanzar 80% del CLV (robusto)
if total_clv > 0 and len(df_pareto) > 0:
    idx = int(np.searchsorted(df_pareto["cum_share"].values, 0.80, side="left"))
    idx = min(idx, len(df_pareto) - 1)
    pct_clients_80 = (idx + 1) / len(df_pareto) * 100
else:
    pct_clients_80 = np.nan
...
# ---------- Slide 2: Pareto
    ...
    max_top = max(1, min(300, len(df_pareto)))
    default_top = min(50, max_top)
    top_n = st.slider("Mostrar Top N clientes en barras (para detalle)", 1, max_top, default_top)
