...
# ---------- Slide 5
    ...
    # KPIs
    k1, k2 = st.columns(2)
    k1.metric("Clientes con respuesta SÍ", f"{resp_yes:.1f}%")
    k2.metric("Clientes con respuesta NO", f"{resp_no:.1f}%")

    # Gráfico pie
    fig5 = px.pie(values=[resp_yes, resp_no], 
                  names=["Yes", "No"], 
...
# ---------- Slide 6
    ...
    st.subheader("🌎 Slide 6 · CLV por Estado (Mapa de calor)")

    # Agrupar CLV por estado
    df_states = df.groupby("State", as_index=False)["Customer Lifetime Value"].sum()
    estado_lider = df_states.loc[df_states["Customer Lifetime Value"].idxmax(), "State"]
    valor_lider = df_states["Customer Lifetime Value"].max()

    # Calcular Top 5
    top_estados = df_states.nlargest(5, "Customer Lifetime Value")
    participacion_top5 = (top_estados["Customer Lifetime Value"].sum() / df["Customer Lifetime Value"].sum()) * 100

    # KPIs
    k1, k2 = st.columns(2)
    k1.metric("Estado líder", estado_lider)
    k2.metric("CLV del líder", f"{valor_lider:,.2f}")

    # Gráfico coroplético USA
    fig6 = px.choropleth(
        df_states,
        locations="State",
        ...
        title="Mapa de calor del CLV por Estado"
    )
    st.plotly_chart(fig6, use_container_width=True)

    # Narrativa
    st.markdown(f"""
    🔎 El estado líder es **{estado_lider}** con CLV total de **{valor_lider:,.2f}**.  
    🔎 Los **Top 5 estados** concentran aproximadamente el **{participacion_top5:.1f}%** del CLV total.
    """)
# ---------- Slide 7
    ...
    cliente_top = top_clientes.iloc[0]["Customer"]
    valor_top = top_clientes.iloc[0]["Customer Lifetime Value"]
    participacion_top10 = (top_clientes["Customer Lifetime Value"].sum() / df["Customer Lifetime Value"].sum()) * 100

    # KPIs
    k1, k2 = st.columns(2)
    k1.metric("Cliente Top", cliente_top)
    k2.metric("CLV del Top", f"{valor_top:,.2f}")
...
# ---------- Slide 8
    ...
    fig8 = px.scatter(df, x="Income", y="Customer Lifetime Value", trendline="ols",
